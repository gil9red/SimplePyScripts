...
# SOURCE: https://github.com/mozman/svgwrite/blob/master/examples/ltattrie/color_names.py
...
def create_svg(name):
    ...
    dwg.add(dwg.text(title1, insert=(0, (font_size + 5)), font_family="serif", font_size=font_size, fill='white'))

    for i, sample_item in enumerate(sample):
        y_loc = 1.5 * font_size * (i + 2)
        dwg.add(dwg.rect(insert=(font_size, (y_loc - font_size)), size=(font_size, font_size), fill=sample_item[0]))
        dwg.add(dwg.text(
            sample_item[0] + " " + sample_item[1],
