...
def create_svg(name):
    ...
    y = font_size + 5
    dwg.add(dwg.text(title, insert=(0, y), font_family="serif", font_size=font_size, fill='white'))
    ...
    for i in range(triangles_per_side, 0, -1):
        ...
        # Calculate color
        start_color = [
            int(tri_color[0][k] * ratio_side[i-1] + tri_color[2][k] * (1 - ratio_side[i-1]))
            for k in range(3)
        ]
        end_color = [
            int(tri_color[1][k] * ratio_side[i-1] + tri_color[2][k] * (1 - ratio_side[i-1]))
            for k in range(3)
        ]
        ...
        for j in range(i):
            ...
            # Calculate color
            new_color = [
                int(start_color[k] * (1 - ratio_row[num_tri_drawn_in_row])
