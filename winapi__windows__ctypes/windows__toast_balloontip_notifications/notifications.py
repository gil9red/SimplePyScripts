...
class WindowsBalloonTip:
    ...
    def balloon_tip(title, msg, duration=5, icon_path_name=None):
        ...
        hwnd = CreateWindow(class_atom, "Taskbar", style, 0, 0,
                            win32con.CW_USEDEFAULT, win32con.CW_USEDEFAULT,
                            0, 0, hinst, None)
        UpdateWindow(hwnd)

        if not icon_path_name:
            icon_path_name = os.path.abspath(os.path.join(sys.path[0], "balloontip.ico"))
        ...
        Shell_NotifyIcon(NIM_MODIFY,
                         (hwnd, 0, NIF_INFO, win32con.WM_USER + 20, hicon, "Balloon  tooltip", msg, 200, title))
