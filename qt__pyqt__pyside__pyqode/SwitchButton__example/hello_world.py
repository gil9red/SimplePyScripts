...
class MainWindow(QWidget):
    def __init__(self):
        ...
        label_switch_btn1 = QLabel(f'Checked: {switch_btn1.checked()}, text: {switch_btn1.valueText()}')
        switch_btn1.clicked.connect(lambda checked: label_switch_btn1.setText(
            f'Checked: {switch_btn1.checked()}, text: {switch_btn1.valueText()}'
        ))

        switch_btn2 = SwitchButton(self, "Вкл", 15, "Откл", 31, width=120)
        label_switch_btn2 = QLabel(f'Checked: {switch_btn2.checked()}, text: {switch_btn2.valueText()}')
        switch_btn2.clicked.connect(lambda checked: label_switch_btn2.setText(
            f'Checked: {switch_btn2.checked()}, text: {switch_btn2.valueText()}'
        ))
