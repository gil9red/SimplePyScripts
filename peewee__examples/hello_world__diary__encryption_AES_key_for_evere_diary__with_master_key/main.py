...
class Diary(BaseModel):
    ...
    def create_encrypted_content(content: str, master_key: str) -> 'Diary':
        ...
        return Diary(encrypted_content=encrypted_content, encrypted_key=encrypted_key).save()
    ...
    def print_table(master_key: str = ENCRYPT_MASTER_KEY):
        ...
        print(header_fmt.format('ID', 'ENCRYPTED_CONTENT', 'CONTENT', 'ENCRYPTED_KEY', 'KEY', 'CREATED_DATE'))
...
if not Diary.select().count():
    Diary.create_encrypted_content(content="Hello World!", master_key=ENCRYPT_MASTER_KEY)
    Diary.create_encrypted_content(content="The quick brown fox jumps over the lazy dog.", master_key=ENCRYPT_MASTER_KEY)
