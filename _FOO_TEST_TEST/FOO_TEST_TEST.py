...
def download_video(url: str) -> Path:
    ...
    _, video_type, video_url = max(items, key=lambda x: x[0])
    print(video_type, video_url)

    file_name = Path(f"{title}.{video_type}")

    from datetime import datetime
    dt = datetime.now()
    print(dt.isoformat(), "Download to:", file_name)
    ...
    print(datetime.now().isoformat(), "Finish download to:", file_name)
    print(f"Elapsed: {datetime.now() - dt}")

    print(sizeof_fmt(file_name.stat().st_size))

    return file_name


url = "https://jut.su/deshi-kenichi/episode-6.html"
url = "https://jut.su/deshi-kenichi/episode-10.html"
url = "https://jut.su/naruuto/season-2/episode-274.html"
url = "https://jut.su/naruuto/season-1/episode-136.html"
