...
def get_video_list_url_from_seasonvar_ru(url):
    ...
    get_playlist_url = 'http://kinogo.club/playlist/{}.txt?rand={}'.format(serial_url_name, random.random())
    ...
    for row in json_data['playlist']:
        if 'file' in row:
            list_of_series.append((row['comment'].encode('cp1251').decode('utf-8'), row['file']))
