...
try:
    ...
    section_after_comments_el = driver.find_element_by_css_selector('.comments-wrapper ~ section')
    ...
    while True:
        ...
        while True:
            try:
                ActionChains(driver).move_to_element(section_after_comments_el).perform()
                break
    ...
    for comment_el in driver.find_elements_by_css_selector('div.page-story__comments div.comment'):
        comment_id = comment_el.get_attribute('id')

        comment_body_el = comment_el.find_element_by_class_name('comment__body')
        user_name = comment_body_el.find_element_by_css_selector('.comment__user').get_attribute('data-name')
        # print(user_name, comment_id)

        images = comment_body_el.find_elements_by_css_selector('div.comment-image__content > a > img')
        num_images += len(images)
        ...
        for i, img_el in enumerate(images, 1):
            ...
            num_img = f'_{i}' if len(images) > 1 else ''
            file_name = dir_name / f'{user_name}__{comment_id}{num_img}{get_suffix(img_url)}'
            urlretrieve(img_url, file_name)
