...
try:
    ...
    current_rep = get_int(driver.find_element_by_css_selector('#user-tab-reputation .count'))
    ...
    while True:
        for tr in driver.find_elements_by_css_selector('#rep-page-container .rep-table-row'):
            ...
            is_expanded = has_class('expander-arrow-small-show', rep_day_el.find_element_by_css_selector('.load-body'))
            if not is_expanded:
                scroll_and_click(rep_day_el, sleep=1.5)

            # От текущего элемента ищем следующий tr с классом loaded-body
            loaded_body_el = tr.find_element_by_xpath("following-sibling::tr[contains(@class, 'loaded-body')]")
            rep_change_items = loaded_body_el.find_elements_by_css_selector('.rep-breakdown-row')
            if rep_change_items:
                sum_negative_rep = 0

                for rep_row_el in rep_change_items:
                    rep_left_value = rep_row_el.find_element_by_css_selector('.rep-left').text.strip()
                    ...
                    DEBUG_LOG and print(f'    {rep_left_value:4} ({rep_up}/{rep_down}) : {rep_desc!r} => {rep_desc_title!r}')
        ...
        try:
            next_el = driver.find_element_by_css_selector('.s-pagination > [rel="next"]')
    ...
    print(f'Total negative reputation: {total_negative_rep} ({percent_negative_rep:.2f}%)')
