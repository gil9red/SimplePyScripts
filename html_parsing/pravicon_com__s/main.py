...
DIR_DUMP = Path(__file__).resolve().parent / 'DUMP_pravicon_com__s'
DIR_DUMP.mkdir(parents=True, exist_ok=True)

SESSION = requests.session()
SESSION.headers['User-Agent'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:75.0) Gecko/20100101 Firefox/75.0'
...
def get_plaintext(element: Tag) -> Optional[str]:
    if not element:
        return
...
def get_main_image_url(tag: Tag) -> Optional[str]:
...
def get_keywords(tag: Tag) -> Optional[str]:
...
def get_days_celebration(tag: Tag) -> Optional[str]:
...
def get_url_more_info(tag: Tag) -> Optional[str]:
...
def get_description(tag: Tag) -> Optional[str]:
...
def get_liturgical_texts(tag: Tag) -> Optional[str]:
...
def get_image_description(tag: Tag) -> Optional[str]:
...
def get_image_url(tag: Tag) -> Optional[str]:
...
def dump_icon(dir_dump: Path, url: str, title: str = None):
    ...
    json.dump(
        data_info,
        open(path_info, 'w', encoding='utf-8'),
        ensure_ascii=False,
        indent=4
    )
