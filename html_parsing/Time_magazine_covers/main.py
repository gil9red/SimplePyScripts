...
def get_covers(year: int | str) -> list[Cover]:
    items = []
    ...
    try:
        ...
        if not rs.ok:
            raise Exception(
                f"Something went wrong...: status_code: {rs.status_code}"
                f"\nUrl: {url}\nRs.Url: {rs.url}\n{rs.text}"
            )
        ...
        if not match:
            raise Exception(
                f'Not found "Time.bootstrap = ...): status_code: {rs.status_code}'
                f'\nUrl: {url}\nRs.Url: {rs.url}\n{rs.text}"'
            )
        ...
        if "vault" not in result:
            raise Exception(
                f'Field "vault" not found in result: status_code: {rs.status_code}'
                f"\nUrl: {url}\nRs.Url: {rs.url}"
                f"\nResult: {result}\nHtml:\n{rs.text}"
            )
