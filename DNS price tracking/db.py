...
class Product(BaseModel):
    ...
    def append_price(self, value_dns, value_technopoint):
        Price.create(product=self, value_dns=value_dns, value_technopoint=value_technopoint)

    def __str__(self):
        # TODO: выводить последнюю цену и актуальную
        return f'Product(title={self.title!r}, ' \
...
class Price(BaseModel):
    ...
    class Meta:
        indexes = (
            (("product_id", "date", "value_dns", "value_technopoint"), True),
        )

    def __str__(self):
        return f'Price(value_dns={self.value_dns}, ' \
