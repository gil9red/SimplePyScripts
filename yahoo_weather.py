...
if __name__ == '__main__':
    city = 'Magnitogorsk'
    query = "select item from weather.forecast where woeid in (select woeid from geo.places where text='{}') and u='c'".format(city)
    ...
    import requests
    rs = requests.get(url)
    item = rs.json()['query']['results']['channel'][0]['item']
