...
class ImageHashDetailsDialog(QDialog):
    def __init__(self, file_name: str, data: dict, parent=None):
        ...
        action_run = value_widget.addAction(QIcon(DIR_IMAGES + '/run_image.svg'), QLineEdit.TrailingPosition)
        action_run.triggered.connect(lambda: explore(value_widget.text(), select=False))

        action_view = value_widget.addAction(QIcon(DIR_IMAGES + '/view.svg'), QLineEdit.TrailingPosition)
