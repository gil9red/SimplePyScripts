...
DATA = {"MODE": ErrorHandlingMode.SILENCE}
...
def reply_error(log: logging.Logger, update: Update, context: CallbackContext) -> None:
    ...
    if DATA["MODE"] in [
        ErrorHandlingMode.SHOW_ERROR,
        ErrorHandlingMode.SHOW_ERROR_WITH_REPORT,
    ]:
        text = f"{text}\n\n{message_report}"
    ...
    if DATA["MODE"] in [
        ErrorHandlingMode.SILENCE_WITH_REPORT,
        ErrorHandlingMode.SHOW_ERROR_WITH_REPORT,
    ]:
        for n in range(0, len(message_report), MESS_MAX_LENGTH):
            mess = message_report[n : n + MESS_MAX_LENGTH]
            context.bot.send_message(
                chat_id=DEVELOPER_CHAT_ID, text=mess, parse_mode=ParseMode.HTML
            )
...
def on_reply_command(update: Update, _: CallbackContext) -> None:
    ...
    message.reply_text(
        f'You select mode: {DATA["MODE"]}', reply_markup=REPLY_KEYBOARD_MARKUP
    )
