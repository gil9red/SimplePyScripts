...
if __name__ == "__main__":
    ...
    print(
        f"Закончено получение данных. Всего пользователей комментировало: {len(user_comment_count_dict)}."
    )
    ...
    for k, v in sorted(
        user_comment_count_dict.items(), key=lambda x: x[1], reverse=True
    ):
        print(f"  {k}: {v}")
