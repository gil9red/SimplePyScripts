...
from main import DIR, FILE_NAME_DUMP, get_vk_session
...
for author_id, authors in current_id_by_names.items():
    ...
    if (name not in authors or len(authors) > 1) and 'DELETED' not in name:
        print(f'Актуализация имени {name!r} у пользователя с id={author_id}, предыдущие имена: {names}')
    ...
    if 'DELETED' in name and len(authors) > 1:
        ...
        print(f'В настоящий момент пользователь удален. Актуализация имени {name!r} '
