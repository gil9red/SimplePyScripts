...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        label_left_motor = QLabel('{}'.format(self.left_motor.value()))
        label_left_motor.setAlignment(Qt.AlignCenter)
        self.left_motor.valueChanged.connect(lambda x: label_left_motor.setText('{}'.format(x)))

        self.right_motor = QSlider(Qt.Horizontal)
        self.right_motor.setRange(0, 65535)
        self.right_motor.setValue(32767)

        label_right_motor = QLabel('{}'.format(self.right_motor.value()))
        label_right_motor.setAlignment(Qt.AlignCenter)
        self.right_motor.valueChanged.connect(lambda x: label_right_motor.setText('{}'.format(x)))
