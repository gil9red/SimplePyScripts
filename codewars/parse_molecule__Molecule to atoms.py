...
def parse_molecule(formula):
    ...
    while '(' in formula:
        ...
        if multiplier:
            ...
            # ['K', '4', 'O', '2'] * 2 -> ['K', '8', 'O', '4']
            tokens = [str(int(token) * multiplier) if token.isdigit() else token for token in tokens]
...
if __name__ == '__main__':
    ...
    assert equals_atomically(parse_molecule("H2O"), {'H': 2, 'O': 1}), "Should parse water"
    assert equals_atomically(parse_molecule("Mg(OH)2"), {'Mg': 1, 'O' : 2, 'H': 2}), "Should parse magnesium hydroxide: Mg(OH)2"
    assert equals_atomically(parse_molecule("K4[ON(SO3)2]2"), {'K': 4,  'O': 14,  'N': 2,  'S': 4}), "Should parse Fremy's salt: K4[ON(SO3)2]2"
