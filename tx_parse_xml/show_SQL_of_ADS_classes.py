...
class ADS:
    ...
    def __bool__(self) -> bool:
        return bool(self.props or self.editors or self.selectors or self.filters or self.sortings)
...
def get_ads_filters(ads_el: ET.Element) -> dict[str, list[str]]:
    ...
    for filter_el in ads_el.findall('./ads:Presentations/ads:Filters/ads:Filter', namespaces=NS):
        ...
        if enabled_sorting_el:
            enabled_sorting_hint_el = enabled_sorting_el.find('./ads:Hint', namespaces=NS)
            if enabled_sorting_hint_el and has_text(enabled_sorting_hint_el):
                obj_by_items[filter_title].append('EnabledSorting/Hint')

            for database in get_databases_specific_sqml_expression(enabled_sorting_el, 'SpecificHint'):
                obj_by_items[filter_title].append(f'EnabledSorting/SpecificHint={database}')
...
def get_ads_props(ads_el: ET.Element) -> dict[str, list[str]]:
    ...
    for prop_el in ads_el.findall('./ads:Properties/ads:Property', namespaces=NS):
        ...
        for database in get_databases_specific_sqml_expression(prop_el, 'SpecificSqmlExpression'):
            prop_by_items[prop_title].append(f'SpecificSqmlExpression={database}')

        parent_select_condition_el = prop_el.find('./ads:Presentation/ads:ParentSelect/ads:ParentSelectCondition', namespaces=NS)
...
def get_ads_editor_presentations(ads_el: ET.Element) -> dict[str, list[str]]:
    ...
    for presentation_el in ads_el.findall('./ads:Presentations/ads:EditorPresentations/ads:EditorPresentation', namespaces=NS):
        editor_title = get_xml_object_title(presentation_el)

        for child_item_ref_el in presentation_el.findall('./ads:ChildExplorerItems/ads:Item/ads:ChildRef', namespaces=NS):
            ...
            if child_sqml_items:
                ...
                obj_by_items[editor_title].append(f'{child_ref_title}: {", ".join(child_sqml_items)}')

        for child_item_entity_el in presentation_el.findall('./ads:ChildExplorerItems/ads:Item/ads:Entity', namespaces=NS):
            ...
            if child_sqml_items:
                ...
                obj_by_items[editor_title].append(f'{child_entity_title}: {", ".join(child_sqml_items)}')
...
def get_ads_selector_presentations(ads_el: ET.Element) -> dict[str, list[str]]:
    ...
    for presentation_el in ads_el.findall('./ads:Presentations/ads:SelectorPresentations/ads:SelectorPresentation', namespaces=NS):
        ...
        if addons_el:
            ...
            for database in get_databases_specific_sqml_expression(specific_hint_el, 'SpecificDefaultHint'):
                obj_by_items[selector_title].append(f'Addons/SpecificDefaultHint={database}')
...
def get_ads_sortings(ads_el: ET.Element) -> dict[str, list[str]]:
    ...
    sorting_el = ads_el.find('./ads:Presentations/ads:Sortings/ads:Sorting', namespaces=NS)
    if sorting_el:
        ...
        for database in get_databases_specific_sqml_expression(specific_hint_el, 'SpecificHint'):
            obj_by_items[sorting_title].append(f'SpecificHint={database}')

    return obj_by_items


def get_ads(model_path: Path) -> Optional[ADS]:
...
def get_ads_list(branch_dir: Union[Path, str]) -> dict[str, list[ADS]]:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Shows ADS entities with SQL definitions")
    parser.add_argument("path_trunk", metavar="/PATH/TO/TRUNK", help="Path to TX source tree (the directory containing branch.xml)")
