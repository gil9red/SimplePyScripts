...
def get_health(boss: Boss, items: Dict[str, List[str]]) -> int:
    ...
    if boss_name == "душа пепла":
        ...
        return only_digits(health_items[first_idx + 1]) + only_digits(
            health_items[second_idx + 1]
        )
    ...
    if boss_name == "хранители бездны":
        first_idx = health_items.index("Первая Фаза")
        second_idx = health_items.index("Вторая Фаза")
        return only_digits(health_items[first_idx + 2]) + only_digits(
            health_items[second_idx + 2]
        )
    ...
    if boss_name == "демон-принц":
        ...
        return only_digits(health_items[first_idx + 2], do_sum="/") + only_digits(
            health_items[second_idx + 2]
        )
    ...
    for category, bosses in get_bosses(url).items():
        ...
        for boss in bosses:
            ...
            try:
                ...
                debug_log and print(
                    f"    {boss.name}. Здоровье: {health}, души: {souls}"
                )
