...
class Parser:
    ...
    def parse(self) -> 'Parser':
        ...
        for a in root.select('.category-page__member-link'):
            ...
            for x_title, x_url in locations:
                self._log('        {} -> {}'.format(x_title, x_url))
            ...
            for boss_name, boss_url in bosses:
                self._log('        {} -> {}'.format(boss_name, boss_url))
    ...
    def get_location_by_bosses(self, location_name: str = None) -> Union[Dict[str, List[Tuple[str, str]]], List[Tuple[str, str]]]:
        if location_name is None:
            return self._location_by_bosses

        return self._location_by_bosses.get(location_name)

    def get_location_by_url(self, location_name: str = None) -> Union[Dict[str, str], str]:
        if location_name is None:
            return self._location_by_url

        return self._location_by_url.get(location_name)

    def get_boss_by_url(self, boss_name: str = None) -> Union[Dict[str, str], str]:
...
if __name__ == '__main__':
    ...
    # DS1
    bosses = get_bosses_by_location('http://ru.darksouls.wikia.com/wiki/Северное_Прибежище_Нежити')
    ...
    # DS3
    bosses = get_bosses_by_location('http://ru.darksouls.wikia.com/wiki/Анор_Лондо_(Dark_Souls_III)')
    print(len(bosses), bosses)
